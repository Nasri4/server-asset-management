  ...
  try {
    ...
    const result = await query(
      `SELECT u.user_id, u.username, u.full_name, u.email, u.phone, u.department_id, u.team_id,
              ...
              r.role_name, r.level as role_level, e.engineer_id
       FROM users u
    ...
    await query(
      `UPDATE users SET failed_attempts = 0, locked_until = NULL, last_login = GETDATE()
       WHERE user_id = @user_id`,
    ...
    await logAudit(user.user_id, user.username, 'LOGIN', 'auth', null, null, null,
