...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, data: store.config["server-statuses"] });
}
...
export async function POST(req: Request) {
  ...
  const existing = store.config["server-statuses"].find((t) => t.name.toLowerCase() === name.toLowerCase());
  if (existing) {
    return NextResponse.json({ ok: false, error: { message: "Already exists" } }, { status: 409 });
  }

  const item = { id: store._seq.configId++, name };
  store.config["server-statuses"].push(item);
